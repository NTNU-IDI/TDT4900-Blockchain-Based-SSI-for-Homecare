...
async function main() {
    ...
    if (PATIENT_ADDRESSES.length === 0) throw new Error("ACCOUNTS are not defined");
    if (IPFS_HASHES.length === 0) throw new Error("IPFS_HASHES are not defined");
    if (PATIENT_ADDRESSES.length !== IPFS_HASHES.length) throw new Error("Each account must have a matching IPFS hash");
    ...
    for (let i = 0; i < PATIENT_ADDRESSES.length; i++) {
        const account = PATIENT_ADDRESSES[i];
